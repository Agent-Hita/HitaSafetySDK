...
class RiskScorer(
    ...
    fun score(text: String, context: List<String> = emptyList()): List<DetectionResult> {
        ...
        // design and their detectors already return a blunt NONE-or-HIGH
        // result on any match. Suppressed entirely for adult accounts;
        // HARMFUL_CONTENT stays HIGH for both CHILD and ADOLESCENT;
        // ADULT_CONTENT stays HIGH for CHILD but caps to MEDIUM for
        // ADOLESCENT. Placed after every step that recomputes riskLevel from
        // score via the age-band-aware scoreToRiskLevel (escalated/
        // otpEscalated above) — doing this earlier let a later recompute
        // silently erase the explicit MEDIUM override, since 0.30 doesn't
        // clear ADOLESCENT's own medium threshold (0.65) even though it's
        // well above the flat 0.28 the detector's own internal
        // scoreToRiskLevel uses. See ResponseOnlyRiskScorer's identical step
        // (and NewContentCategoriesScorerTest) for the full story.
        ...
        val merged: Map<HarmCategory, DetectionResult> = if (classifierResult == null) {
            ageGatedNewCategories
        } else {
            val (classifierCat, classifierSeverity) = classifierResult
            ageGatedNewCategories.mapValues { (cat, result) ->
