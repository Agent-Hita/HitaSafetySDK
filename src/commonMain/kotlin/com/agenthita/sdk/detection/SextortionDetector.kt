...
class SextortionDetector : PatternMatcher {
    ...
    // Victim-voiced refusal or hesitancy toward an image/sexual advance —
    // the request itself is invisible to a response-only caller, so these
    // catch the *reply* as evidence a solicitation happened even though
    // declining is the safe outcome. Added 2026-09-10 alongside the
    // analogous moneyRefusalSignals in FinancialScamDetector — same
    // "response reveals an unseen interaction" pattern. Weighted like
    // secrecy (single match alone lands at LOW/CAUTION, not MEDIUM/HIGH).
    private val advanceRefusalSignals = listOf(
        "please stop asking me for pictures", "stop asking me for pictures",
        "please stop asking me for photos", "stop asking me for photos",
        "please stop asking for pictures", "please stop asking for photos",
        "please stop asking me to send pics", "stop asking me to send nudes",
        "please stop asking for nudes", "stop asking for nudes",
        "i will not send you pictures", "i won't send you pictures",
        "i will not send you photos", "i won't send you photos",
        "i will not send pics", "i won't send pics",
        "i will not send nudes", "i won't send nudes",
        "i do not want to send that photo", "i don't want to send that photo",
        "i do not want to send that picture", "i don't want to send that picture",
        // Hesitancy, not just flat refusal
        "i am not comfortable sending pictures", "i'm not comfortable sending pictures",
        "i am not comfortable sending that", "i'm not comfortable sending that",
        "i do not feel comfortable sending photos", "i don't feel comfortable sending photos",
        "i am not comfortable with that", "i'm not comfortable with that",
        "i am not sure i should send that", "i'm not sure i should send that",
        "i do not think i should send that", "i don't think i should send that"
    )
    ...
    override fun analyze(text: String): DetectionResult {
        ...
        advanceRefusalSignals.forEach { signal ->
            if (lower.contains(normalizeContractions(signal))) matches.add(SignalMatch("advance_refusal", signal, 0.65f))
        }
